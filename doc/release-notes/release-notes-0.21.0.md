...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in WHIVE per 1,000
  vbytes (WHIVE/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from WHIVE/kB
  to sat/vB and the help documentation in WHIVE/kB is updated to WHIVE/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from WHIVE/kvB to sat/vB and the
  ...
  difference between WHIVE/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in WHIVE/kvB rather than sat/vB should raise an
