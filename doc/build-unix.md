...
To build Whive Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/whive/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/whive/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Whive Core can
