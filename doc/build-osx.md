...
This guide describes how to build whived, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Whive Core from source.
...
### 4. Clone Whive repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Whive Core repository to a directory.
...
It is not necessary to build wallet functionality to run `whived` or  `whive-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/whiveio/whive/blob/master/doc/descriptors.md), `sqlite` is required.
...
Whive Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Whive Core application using `make deploy`.
...
There are many ways to configure Whive Core, here are a few common examples:
...
Run the following in your terminal to compile Whive Core:
...
Whive Core should now be available at `./src/whived`.
If you compiled support for the GUI, it should be available at `./src/qt/whive-qt`.

The first time you run `whived` or `whive-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Whive/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Whive"
...
./src/qt/whive-qt -server # Starts the whive-qt server mode, allows whive-cli control
```
