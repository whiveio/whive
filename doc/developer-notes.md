...
- [Development guidelines](#development-guidelines)
    - [General Whive Core](#general-whive-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Whive Core is a multi-threaded application, and deadlocks or other
...
Whive Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Whive Core, where everyone uses
...
pay attention to for reviewers of Whive Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Whive Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Whive Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Whive Core so that they can be integrated
...
file descriptors it will cause problems with Whive's `select()` loop, because
...
compatibility between nodes. This happened in Whive 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Whive's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:whiveio/whive.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `whive-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `whive-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Whive Core, the wallet was always
