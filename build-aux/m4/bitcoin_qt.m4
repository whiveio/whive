...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$whive_qt_want_version" = "auto" && test "$whive_qt_force" != "yes"; then
    if test "$whive_enable_qt" != "no"; then
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$whive_enable_qt" != "no" && test "$whive_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "$whive_qt_want_version" = "yes"; then
       whive_qt_force=yes
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$whive_cv_static_qt" = "yes"; then
  ...
  if test $whive_enable_qt = "yes"; then
...
dnl Output: whive_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, whive_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [whive_cv_static_qt=yes],
      [whive_cv_static_qt=no])
    ])
])
